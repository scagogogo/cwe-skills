...
```mermaid
classDiagram
    CWEError <|-- InvalidCWEIDError
    CWEError <|-- CWENotFoundError
    CWEError <|-- APIError
    CWEError <|-- ValidationError
    CWEError <|-- ParseError
    CWEError <|-- RelationshipError
    ...
    class InvalidCWEIDError {
        +ID string
    }
    class CWENotFoundError {
        +ID int
    }
    class APIError {
        +StatusCode int
...
```mermaid
flowchart TD
    ERR(["err != nil"]) --> AS{"errors.As 匹配"}
    AS -- "CWENotFoundError" --> NF["fallback / 提示不存在\n用 %w 包装返回"]
    AS -- "RateLimitError" --> RL["等待 RetryAfter\n自定义重试"]
    AS -- "APIError 且 >=500" --> S5["提示服务端故障\n稍后重试"]
    AS -- "APIError 4xx" --> A4["检查请求参数"]
    AS -- "其他" --> FALL["兜底\nfmt.Errorf 查询失败: %w"]
