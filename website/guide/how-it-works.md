...
sequenceDiagram
    ...
    participant MITRE as MITRE REST API

    Caller->>API: GetWeakness(ctx, 79)
    API->>HTTP: DoRequest(ctx, ...)
    HTTP->>RL: Allow() / Wait(ctx)
    Note over RL: 令牌桶 默认 0.1/s burst=1
    RL-->>HTTP: 放行
    HTTP->>MITRE: GET /cwe/79
    MITRE-->>HTTP: JSON 响应
    alt 状态码 >= 500
        HTTP->>HTTP: 指数退避重试 (maxRetries)
        HTTP->>MITRE: 重试请求
    end
    HTTP->>HTTP: 解析 JSON → *CWE
    HTTP-->>API: *CWE / *Category / *View / []Relationship
    API-->>Caller: 返回结果
...
```mermaid
flowchart TB
    START["调用方"] --> XML["XMLParser.ParseFile"]

    subgraph PARSE["解析阶段"]
        XML --> DECODE["encoding/xml 流式解析"]
        DECODE --> REG["构建空 Registry"]
        REG --> REGOP["逐条 Register/RegisterCategory/RegisterView/RegisterCompoundElement"]
    end

    REGOP --> BUILD["BuildIndexes()"]

    subgraph INDEX["索引阶段"]
        BUILD --> PARENT["parentIndex 子→父"]
        BUILD --> CHILD["childIndex 父→子"]
        BUILD --> PEER["peerIndex 对等"]
        BUILD --> MEMBER["memberIndex 类/视图→成员"]
        BUILD --> MEMBEROF["memberOfIndex 成员→所属"]
    end

    subgraph CONSUME["消费阶段"]
        INDEX --> NAV["Navigator\nAncestors / ShortestPath"]
        INDEX --> TREE["BuildTree\n层次树 / 森林 / 视图树"]
        INDEX --> FIND["FindBy* / Filter\n搜索过滤"]
        INDEX --> EXPORT["ExportJSON/CSV\n序列化导出"]
    end

    classDef offline fill:#ffedd5,stroke:#ea580c,color:#9a3412
    classDef core fill:#e8f1f8,stroke:#3c6c8f,color:#1d3a4f
    class XML,PARSE,DECODE,REG,REGOP,BUILD,INDEX,PARENT,CHILD,PEER,MEMBER,MEMBEROF offline
    class START,CONSUME,NAV,TREE,FIND,EXPORT core
```
