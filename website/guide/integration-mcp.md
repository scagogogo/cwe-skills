...
<Badge type="tip" text="已实现" /> <Badge type="info" text="stdio + SSE" /> <Badge type="warning" text="v0.1.0" />

::: tip 已发布
MCP 服务器 `cwe-mcp` 已实现，暴露 **15 个工具**，覆盖 ID 工具、知名列表、在线 API、离线 XML 导航/树/搜索。支持 stdio（本地客户端）与 SSE（远程）两种传输。
:::
...
MCP 服务器暴露 15 个工具，与 CLI 子命令一一对应：
...
::: tip 在线 + 离线组合
MCP 服务器可同时启用在线与离线工具。典型用法：`get_weakness`（在线，最新详情）+ `get_ancestors`（离线，完整关系）。
:::
