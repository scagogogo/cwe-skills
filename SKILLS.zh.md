...
> `cwe nav` 共有 17 个子命令——还有 `can-also-be`、`chain-members`、`composite-members`、`follow`、`is-related`、`precede`、`requires`、`required-by` 覆盖其他 CWE 关系类型。运行 `cwe nav --help` 查看全部。`cwe tree view` 构建视图作用域树；`cwe registry` 有 15 个子命令用于类别/视图成员查询。
